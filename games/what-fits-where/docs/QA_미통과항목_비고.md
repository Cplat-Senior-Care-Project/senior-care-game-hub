...
| 결과 화면 | 케어/AI연동 모드 | config에 따라 자동 복귀 또는 AI 대화 세션 복귀가 정상 동작하는가 | 부분 통과 | `care`, `ai_assisted` 기본 config에 `auto_return_to_hub:true`가 있고(`config/modes.js:146-187`), 결과 화면 후 `returnToHub()`가 1.2초 뒤 호출됨(`js/game.js:2399-2401`). 단 AI 대화 세션 전용 복귀 이벤트는 별도 구현 없이 `RETURN_TO_HUB`/`RETURN_TO_HYODAM_CALL`만 전송함(`js/game.js:2155-2167`). |
