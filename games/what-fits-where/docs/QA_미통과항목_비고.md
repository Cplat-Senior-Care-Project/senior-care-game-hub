...
| 공통/앱 연동 | 모든 모드 | 모드별 config(show_timer, show_score, hint_enabled, voice_guide_enabled 등)가 정상 반영되는가 | 부분 통과 | `show_timer`, `show_pause`, `show_hint`, `show_score`, `voice_guide_enabled` 등은 config와 UI/결과 흐름에 반영됨(`config/modes.js:62-68`, `js/game.js:247-273`, `js/game.js:191-194`, `js/game.js:1677-1685`). 다만 명세의 `hint_enabled`가 아니라 `show_hint`를 사용함. |
